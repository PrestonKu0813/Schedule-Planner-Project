...
const renderOnlineSectionItem = (
  ...
  return (
    <div
      ...
      title={
        isClosed ? `${course.course_name} - Section Closed` : course.course_name
      }
    >
      <p>
      ...
      {isClosed && (
        <p style={{ color: "#dc3545", fontSize: "12px", margin: "2px 0" }}>
          CLOSED
        </p>
      )}
    </div>
...
function Calendar({ courses, previewSection }) {
  ...
  const handleClassClick = (cls, event) => {
    if (calendarRef.current) {
      ...
      // More accurate popup dimensions based on CSS and content
      const POPUP_WIDTH = 200; // matches max-w-[200px] from CSS
      const POPUP_HEIGHT = 200; // reduced to be more conservative
      ...
      // Check if popup would overflow the container bottom (with some buffer)
      const containerBottom = containerRect.height;
      const popupBottomInContainer = popupY + POPUP_HEIGHT;
      const buffer = 100; // Increased buffer to trigger overflow detection earlier

      // If popup would overflow the container bottom, position it above the class
      if (popupBottomInContainer > containerBottom - buffer) {
      ...
      if (popupY < 0) {
        popupY = 5; // Small margin from top
      }
  ...
  const isSectionClosed = (section) => {
    return (
      section.section_open !== undefined &&
      section.section_open !== null &&
      section.section_open === 0
    );
  };
  ...
  const renderLecture = (
    ...
    const { topPos, heightPos, dayLeft, start, end } =
      calculateLecturePosition(lecture);
    const classDetails = createClassDetails(
      course,
      section,
      lecture,
      start,
    ...
    const style = {
      ...
      backgroundColor: isClosed
        ? "#808080"
        : campusColors[lecture.campus] || "#ccc",
      ...(isPreview && {
        opacity: 0.7,
        zIndex: 1000,
      }),
    ...
    return (
      <div
        ...
        style={style}
        onClick={
          isPreview ? undefined : (e) => handleClassClick(classDetails, e)
        }
        title={
          isClosed
            ? `${course.course_name} - Section Closed`
            : course.course_name
        }
      >
      ...
      <div className="calendar_body">
        ...
        {previewSection &&
          (() => {
            ...
            const lectureInfoArray = Object.values(
              previewSection.section.lecture_info
            );
      ...
      {(() => {
        ...
        return (
          <>
            ...
            >
              {isOnlinePopupVisible ? "▼" : "▲"} Online Classes (
              {onlineSections.length})
            </button>
